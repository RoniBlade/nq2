...
CREATE OR REPLACE FUNCTION glt_texttoarrayjsonb(var text)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$
DECLARE
flagArray TEXT;
BEGIN
	flagArray:=left(var,1);
		if flagArray !='[' then
			var:=concat('[',var,']');
		end if;

        RETURN cast (var as jsonb);
END;
$function$;

CREATE OR REPLACE VIEW glt_object_info_lite
AS SELECT row_number() OVER () AS id,
    mo.oid,
    mo.objecttype::text AS objecttype,
        CASE
            WHEN mo.objecttype = 'ROLE'::objecttype THEN ((convert_from(mo.fullobject, 'UTF8'::name)::json ->> 'role'::text)::json) ->> 'name'::text
            WHEN mo.objecttype = 'USER'::objecttype THEN ((convert_from(mo.fullobject, 'UTF8'::name)::json ->> 'user'::text)::json) ->> 'name'::text
            WHEN mo.objecttype = 'ORG'::objecttype THEN ((convert_from(mo.fullobject, 'UTF8'::name)::json ->> 'org'::text)::json) ->> 'name'::text
            WHEN mo.objecttype = 'SERVICE'::objecttype THEN ((convert_from(mo.fullobject, 'UTF8'::name)::json ->> 'service'::text)::json) ->> 'name'::text
            ELSE NULL::text
        END AS object_name,
        CASE
            WHEN mo.objecttype = 'ROLE'::objecttype THEN ((convert_from(mo.fullobject, 'UTF8'::name)::json ->> 'role'::text)::json) ->> 'displayName'::text
            WHEN mo.objecttype = 'USER'::objecttype THEN ((convert_from(mo.fullobject, 'UTF8'::name)::json ->> 'user'::text)::json) ->> 'fullName'::text
            WHEN mo.objecttype = 'ORG'::objecttype THEN ((convert_from(mo.fullobject, 'UTF8'::name)::json ->> 'org'::text)::json) ->> 'displayName'::text
            WHEN mo.objecttype = 'SERVICE'::objecttype THEN ((convert_from(mo.fullobject, 'UTF8'::name)::json ->> 'service'::text)::json) ->> 'displayName'::text
            ELSE NULL::text
        END AS object_fullname,
        CASE
            WHEN mo.objecttype = 'ROLE'::objecttype THEN ((convert_from(mo.fullobject, 'UTF8'::name)::json ->> 'role'::text)::json) ->> 'description'::text
            WHEN mo.objecttype = 'USER'::objecttype THEN ((convert_from(mo.fullobject, 'UTF8'::name)::json ->> 'user'::text)::json) ->> 'documentation'::text
            WHEN mo.objecttype = 'ORG'::objecttype THEN ((convert_from(mo.fullobject, 'UTF8'::name)::json ->> 'org'::text)::json) ->> 'documentation'::text
            WHEN mo.objecttype = 'SERVICE'::objecttype THEN ((convert_from(mo.fullobject, 'UTF8'::name)::json ->> 'service'::text)::json) ->> 'description'::text
            ELSE NULL::text
        END AS object_description,
        CASE
            WHEN mo.objecttype = 'USER'::objecttype THEN ((convert_from(mo.fullobject, 'UTF8'::name)::json ->> 'user'::text)::json) ->> 'title'::text
            ELSE NULL::text
        END AS user_title,
        CASE
            WHEN mo.objecttype = 'USER'::objecttype THEN ((convert_from(mo.fullobject, 'UTF8'::name)::json ->> 'user'::text)::json) ->> 'emailAddress'::text
            ELSE NULL::text
        END AS user_emailaddress,
        CASE
            WHEN mo.objecttype = 'USER'::objecttype THEN ((convert_from(mo.fullobject, 'UTF8'::name)::json ->> 'user'::text)::json) ->> 'telephoneNumber'::text
            ELSE NULL::text
        END AS user_telephonenumber,
    mu.photo AS user_photo
   FROM m_object mo
     LEFT JOIN m_user mu ON mo.oid = mu.oid;


   CREATE OR REPLACE VIEW glt_my_request_v
     AS SELECT row_number() OVER () AS id,
         mc.oid AS caseoid,
         ...
         mo.objecttype::text AS objecttype,
             ...
             CASE
                 WHEN mo.objecttype = 'ORG'::objecttype THEN ((convert_from(mo.fullobject, 'UTF8'::name)::json ->> 'org'::text)::json) ->> 'description'::text
                 WHEN mo.objecttype = 'ROLE'::objecttype THEN ((convert_from(mo.fullobject, 'UTF8'::name)::json ->> 'role'::text)::json) ->> 'description'::text
                 ELSE NULL::text
        ...
        FROM m_case mc,
         ...
         m_user mu1,
         m_object mo
       WHERE mc.targetreftargetoid = mo.oid AND mc.parentreftargetoid IS NOT NULL AND mc.objectreftargetoid = mu.oid AND mc.requestorreftargetoid = mu1.oid;


-- public.glt_case_info исходный текст

CREATE OR REPLACE VIEW glt_case_info
AS WITH combined_data AS (
         SELECT 1 AS part_order,
            ...
            NULL::text AS event_timestamp,
            NULL::text AS event_initiator_oid,
            NULL::text AS event_initiator_name,
            NULL::text AS event_initiator_displayname,
            NULL::text AS event_initiator_relation,
            NULL::text AS event_initiator_type,
            NULL::text AS event_stagenumber,
            NULL::text AS event_delegationmethod,
            NULL::text AS event_externalworkitemid,
            NULL::text AS event_workitemid,
            NULL::text AS event_escalation_level_number,
            NULL::text AS event_escalation_level_name,
            NULL::text AS event_delegated_to_oid,
            NULL::text AS event_delegated_to_relation,
            NULL::text AS event_delegated_to_type,
            NULL::text AS event_delegated_name,
            NULL::text AS event_delegated_displayname,
            NULL::text AS event_cause_type,
            NULL::text AS event_output_outcome,
            NULL::integer AS stage_id,
            NULL::text AS stage_name,
            NULL::integer AS stage_number,
            NULL::text AS stage_groupexpansion,
            NULL::text AS stage_evaluationstrategy,
            NULL::text AS stage_outcomeifnoapprovers,
            NULL::text AS stage_approver_oid,
            NULL::text AS stage_approver_name,
            NULL::text AS stage_approver_displayname,
            NULL::text AS stage_approver_type,
            NULL::text AS stage_approver_relation,
            mcw.cid AS assign_wi_oid,
            mot.oid AS assign_user_oid,
            mu.namenorm AS assign_namenorm,
            mu.fullnameorig AS assign_fullnameorig,
            mc.state AS assign_state,
            mcw.outcome AS assign_outcome
           FROM m_case_wi_assignee mcwa,
            ...
            m_user mu1,
            m_object mo
          WHERE mcwa.owneroid = mcw.owneroid AND mcwa.workitemcid = mcw.cid AND mcwa.owneroid = mc.oid AND mcwa.targetoid = mot.oid AND mc.targetreftargetoid = mo.oid AND mc.parentreftargetoid IS NOT NULL AND mc.objectreftargetoid = mu.oid AND mc.requestorreftargetoid = mu1.oid
        UNION ALL
          SELECT 2 AS part_order,
            row_number() OVER () AS row_num,
            m_case.oid AS caseoid,
            m_case.db_created AS casedatecreate,
            'STAGE'::text AS type,
            NULL::text AS event_type,
            NULL::text AS event_id,
            NULL::text AS event_timestamp,
            ...
            NULL::text AS event_output_outcome,
            x."@id" AS stage_id,
            x.name AS stage_name,
            x.number AS stage_number,
            x."groupExpansion" AS stage_groupexpansion,
            x."evaluationStrategy" AS stage_evaluationstrategy,
            x."outcomeIfNoApprovers" AS stage_outcomeifnoapprovers,
            ar.oid AS stage_approver_oid,
                CASE
                    WHEN ar.type = 'c:UserType'::text THEN ( SELECT mu0.namenorm
                       FROM m_user mu0
                      WHERE mu0.oid = ar.oid::uuid)
                    WHEN ar.type = 'c:RoleType'::text THEN ( SELECT ((convert_from(mr0.fullobject, 'UTF8'::name)::json ->> 'role'::text)::json) ->> 'name'::text
                       FROM m_role mr0
                      WHERE mr0.oid = ar.oid::uuid)
                    ELSE NULL::text
                END AS stage_approver_name,
                CASE
                    WHEN ar.type = 'c:UserType'::text THEN ( SELECT mu0.fullnameorig
                       FROM m_user mu0
                      WHERE mu0.oid = ar.oid::uuid)
                    WHEN ar.type = 'c:RoleType'::text THEN ( SELECT ((convert_from(mr0.fullobject, 'UTF8'::name)::json ->> 'role'::text)::json) ->> 'displayName'::text
                       FROM m_role mr0
                      WHERE mr0.oid = ar.oid::uuid)
                    ELSE NULL::text
                END AS stage_approver_displayname,
            ar.type AS stage_approver_type,
            ar.relation AS stage_approver_relation,
            NULL::bigint AS assign_wi_oid,
            NULL::uuid AS assign_user_oid,
            NULL::text AS assign_namenorm,
            NULL::text AS assign_fullnameorig,
            NULL::text AS assign_state,
            NULL::text AS assign_outcome
           FROM m_case,
            LATERAL jsonb_to_recordset(glt_texttoarrayjsonb(((((((convert_from(m_case.fullobject, 'UTF8'::name)::json ->> 'case'::text)::json) ->> 'approvalContext'::text)::json) ->> 'approvalSchema'::text)::json) ->> 'stage'::text)) x("@id" integer, "@ns" text, name text, number integer, "groupExpansion" text, "evaluationStrategy" text, "outcomeIfNoApprovers" text, "approverRef" text)
            LEFT JOIN LATERAL jsonb_to_recordset(glt_texttoarrayjsonb(x."approverRef")) ar(oid text, relation text, type text) ON true
            where ar.oid is not null
        UNION ALL
         SELECT 3 AS part_order,
            row_number() OVER () AS row_num,
            m_case.oid AS caseoid,
            m_case.db_created AS casedatecreate,
            'EVENT'::text AS type,
            ...
            event."timestamp"::text AS event_timestamp,
            initiator.oid AS event_initiator_oid,
                CASE
                    WHEN initiator.type = 'c:UserType'::text THEN ( SELECT mu0.namenorm
                       FROM m_user mu0
                      WHERE mu0.oid = initiator.oid::uuid)
                    WHEN initiator.type = 'c:RoleType'::text THEN ( SELECT ((convert_from(mr0.fullobject, 'UTF8'::name)::json ->> 'role'::text)::json) ->> 'name'::text
                       FROM m_role mr0
                      WHERE mr0.oid = initiator.oid::uuid)
                    ELSE NULL::text
                END AS event_initiator_name,
                CASE
                    WHEN initiator.type = 'c:UserType'::text THEN ( SELECT mu0.fullnameorig
                       FROM m_user mu0
                      WHERE mu0.oid = initiator.oid::uuid)
                    WHEN initiator.type = 'c:RoleType'::text THEN ( SELECT ((convert_from(mr0.fullobject, 'UTF8'::name)::json ->> 'role'::text)::json) ->> 'displayName'::text
                       FROM m_role mr0
                      WHERE mr0.oid = initiator.oid::uuid)
                    ELSE NULL::text
                END AS event_initiator_displayname,
            initiator.relation AS event_initiator_relation,
            initiator.type AS event_initiator_type,
            event."stageNumber"::text AS event_stagenumber,
            event."delegationMethod" AS event_delegationmethod,
            event."externalWorkItemId" AS event_externalworkitemid,
            event."workItemId"::text AS event_workitemid,
            escalation.number::text AS event_escalation_level_number,
            escalation.name AS event_escalation_level_name,
            delegated.oid AS event_delegated_to_oid,
            delegated.relation AS event_delegated_to_relation,
            delegated.type AS event_delegated_to_type,
                CASE
                    WHEN delegated.type = 'c:UserType'::text THEN ( SELECT mu0.namenorm
                       FROM m_user mu0
                      WHERE mu0.oid = initiator.oid::uuid)
                    WHEN delegated.type = 'c:RoleType'::text THEN ( SELECT ((convert_from(mr0.fullobject, 'UTF8'::name)::json ->> 'role'::text)::json) ->> 'name'::text
                       FROM m_role mr0
                      WHERE mr0.oid = delegated.oid::uuid)
                    ELSE NULL::text
                END AS event_delegated_name,
                CASE
                    WHEN delegated.type = 'c:UserType'::text THEN ( SELECT mu0.fullnameorig
                       FROM m_user mu0
                      WHERE mu0.oid = initiator.oid::uuid)
                    WHEN delegated.type = 'c:RoleType'::text THEN ( SELECT ((convert_from(mr0.fullobject, 'UTF8'::name)::json ->> 'role'::text)::json) ->> 'displayName'::text
                       FROM m_role mr0
                      WHERE mr0.oid = delegated.oid::uuid)
                    ELSE NULL::text
                END AS event_delegated_displayname,
            cause.type AS event_cause_type,
            output.outcome AS event_output_outcome,
            NULL::integer AS stage_id,
            NULL::text AS stage_name,
            NULL::integer AS stage_number,
            NULL::text AS stage_groupexpansion,
            ...
            NULL::text AS assign_outcome
           FROM m_case,
            LATERAL jsonb_to_recordset(glt_texttoarrayjsonb(((convert_from(m_case.fullobject, 'UTF8'::name)::json ->> 'case'::text)::json) ->> 'event'::text)) event("@ns" text, "@type" text, "@id" integer, "timestamp" timestamp with time zone, "initiatorRef" jsonb, "stageNumber" integer, "newEscalationLevel" jsonb, "delegationMethod" text, "delegatedTo" jsonb, cause jsonb, "externalWorkItemId" text, "workItemId" integer, "escalationLevel" jsonb, output jsonb)
             LEFT JOIN LATERAL jsonb_to_record(event."initiatorRef") initiator(oid text, relation text, type text) ON true
             LEFT JOIN LATERAL jsonb_to_record(event."escalationLevel") escalation(number integer, name text) ON true
             LEFT JOIN LATERAL jsonb_to_record(event."delegatedTo") delegated(oid text, relation text, type text) ON true
             LEFT JOIN LATERAL jsonb_to_record(event.cause) cause(type text) ON true
             LEFT JOIN LATERAL jsonb_to_record(event.output) output(outcome text) ON true
        )
 SELECT row_number() OVER (ORDER BY combined_data.part_order, combined_data.row_num) AS id,
    combined_data.caseoid,
    combined_data.casedatecreate,
    combined_data.type,
    combined_data.event_type,
    combined_data.event_id,
    combined_data.event_timestamp,
    combined_data.event_initiator_oid,
    combined_data.event_initiator_name,
    combined_data.event_initiator_displayname,
    combined_data.event_initiator_relation,
    combined_data.event_initiator_type,
    combined_data.event_stagenumber,
    combined_data.event_delegationmethod,
    combined_data.event_externalworkitemid,
    combined_data.event_workitemid,
    combined_data.event_escalation_level_number,
    combined_data.event_escalation_level_name,
    combined_data.event_delegated_to_oid,
    combined_data.event_delegated_to_relation,
    combined_data.event_delegated_to_type,
    combined_data.event_delegated_name,
    combined_data.event_delegated_displayname,
    combined_data.event_cause_type,
    combined_data.event_output_outcome,
    combined_data.stage_id,
    combined_data.stage_name,
    combined_data.stage_number,
    combined_data.stage_groupexpansion,
    combined_data.stage_evaluationstrategy,
    combined_data.stage_outcomeifnoapprovers,
    combined_data.stage_approver_oid,
    combined_data.stage_approver_name,
    combined_data.stage_approver_displayname,
    combined_data.stage_approver_type,
    combined_data.stage_approver_relation,
    combined_data.assign_wi_oid,
    combined_data.assign_user_oid,
    combined_data.assign_namenorm,
    combined_data.assign_fullnameorig,
    combined_data.assign_state,
    combined_data.assign_outcome
   FROM combined_data;

CREATE OR REPLACE VIEW glt_approval_role_info
AS SELECT row_number() OVER () AS id,
    ma.owneroid,
   ...
   FROM m_assignment ma,
    m_object mo
  WHERE ma.targetreftargettype = 'ROLE'::objecttype AND ma.ownertype = 'ROLE'::objecttype AND ma.targetreftargetoid = mo.oid;

CREATE OR REPLACE VIEW glt_approval_request_v
AS SELECT row_number() OVER () AS id,
    mc.oid AS caseoid,
    ...
    ((convert_from(mo.fullobject, 'UTF8'::name)::json ->> 'role'::text)::json) ->> 'displayName'::text AS objectdisplayname,
    ((convert_from(mo.fullobject, 'UTF8'::name)::json ->> 'role'::text)::json) ->> 'description'::text AS objectdescription,
    mo.objecttype::text AS objecttype,
    mc.state,
   ...
   FROM m_case_wi_assignee mcwa,
    ...
    m_user mu1,
    m_object mo
  WHERE mcwa.owneroid = mcw.owneroid AND mcwa.workitemcid = mcw.cid AND mcwa.owneroid = mc.oid AND mcwa.targetoid = mot.oid AND mc.targetreftargetoid = mo.oid AND mcw.outcome IS NULL AND mc.objectreftargetoid = mu.oid AND mc.requestorreftargetoid = mu1.oid AND mc.state <> 'closed'::text;

CREATE OR REPLACE VIEW glt_access_cert_active_request_v AS
SELECT row_number() OVER () AS id,
   ...
   FROM ( SELECT DISTINCT macc.oid AS campaignoid,
          ...
          WHERE maccas.owneroid = macc.oid
          AND macw.owneroid = maccas.owneroid
...
CREATE TABLE d_config_param (
	"oid" uuid NOT NULL,
	oid_parentconfigparam uuid NULL,
	configparam text NOT NULL,
	value text NULL,
	enabled bool NULL,
...
INSERT INTO d_config_param ("oid",oid_parentconfigparam,configparam,value,enabled,description,fullpath,sortorder) VALUES
	 ('2f474b2a-6743-4932-8689-e5f20a03dfb2'::uuid,NULL,'tables',NULL,true,'Переменные для конфигурации отображения данных таблиц','tables',0),
    	 ('a8731eb8-2173-4823-b9d1-b128adb7fc21'::uuid,'2f474b2a-6743-4932-8689-e5f20a03dfb2'::uuid,'m_user',NULL,true,'Таблица пользователей','tables\m_user',1),
    	 ('cfec36a3-33d5-4157-952c-dbecce7ec96e'::uuid,'a8731eb8-2173-4823-b9d1-b128adb7fc21'::uuid,'displaypattern','fullnameorig ||'' (''||nameorig||'')''',true,'шаблон отображаемого имени','tables\m_user\displaypattern',2),
    	 ('00242a71-1c3d-486f-b6b7-a34f84231ea2'::uuid,NULL,'shema',NULL,true,'Переменные схемы','shema',0),
    	 ('bf1bfbfa-38be-4a9b-816e-a26640ddd934'::uuid,'00242a71-1c3d-486f-b6b7-a34f84231ea2'::uuid,'ext_shema','http://example.com/xml/ns/mySchema#',true,'Qname схемы расширения атрибутоы','shema\ext_shema',1);


CREATE OR REPLACE FUNCTION glt_get_displayname(oid text)
RETURNS TABLE (value text)
AS $$
DECLARE
    pattern text;
BEGIN
    -- Явное указание источника данных через алиас
    SELECT c.value INTO pattern
    FROM d_config_param c
    WHERE c.fullpath = 'tables\m_user\displaypattern';  -- Исправлена опечатка fullpatch -> fullpath

    -- Проверка наличия шаблона
    IF pattern IS NULL THEN
        RAISE EXCEPTION 'Display pattern not found';
    END IF;

    -- Корректное формирование динамического SQL
    RETURN QUERY EXECUTE format(
        'SELECT %s FROM m_user WHERE oid::text = %L',
        pattern,
        oid
    );
END;
...
CREATE OR REPLACE FUNCTION glt_get_config_value(fullpathvariable text)
RETURNS  text
AS $$
DECLARE
    var text;
BEGIN
    -- Явное указание источника данных через алиас
    SELECT c.value INTO var
    FROM d_config_param c
    WHERE c.fullpath = fullpathvariable;
    -- Проверка наличия шаблона
    IF var IS NULL THEN
        RAISE EXCEPTION 'Display pattern not found';
    END IF;
...
CREATE OR REPLACE VIEW glt_get_user_profile
AS SELECT row_number() OVER () AS id,
"oid",
nameorig,
namenorm,
...
organizations,
organizationunits,
glt_get_displayname(oid::text) as displayname
FROM public.m_user;



CREATE OR REPLACE VIEW glt_getuser_profile_ext
AS SELECT row_number() OVER () AS id,
	m_user."oid" ,
    xx.value AS ext_attr_value,
    replace(mei.itemname,  glt_get_config_value('shema\ext_shema'), ''::text) AS ext_attr_name,
    mei.cardinality,
    SPLIT_PART(mei.valuetype,'#',2) type,
        CASE
