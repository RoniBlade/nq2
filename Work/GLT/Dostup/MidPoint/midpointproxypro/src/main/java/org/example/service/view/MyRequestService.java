...
import org.example.repository.jdbc.MyRequestJdbcRepository;
...
import static org.example.util.JdbcRows.asString;
...
public class MyRequestService {
    ...
    private final MyRequestJdbcRepository jdbcRepository;

    private static final ObjectMapper MAPPER = new ObjectMapper();

    public Page<MyRequestDto> getMyRequestViaJdbc(Pageable pageable) {
        return jdbcRepository.findAll(pageable).map(row -> toDto(row, null));
    }

    public Page<MyRequestDto> searchMyRequestViaJdbc(FilterRequest request, Pageable pageable, String language) {
    ...
    public Page<MyRequestDto> getMyRequestForUserViaJdbc(Pageable pageable, UUID oid) {
        return jdbcRepository.findAll(pageable).map(row -> toDto(row, null));
    }

    public Page<MyRequestDto> searchMyRequestForUserViaJdbc(FilterRequest request, Pageable pageable, UUID oid) {
        return jdbcRepository.findByFilters(request.getFilters(), pageable)
                .map(row -> DtoFieldTrimmer.trim(toDto(row, null), request.getFields(), request.getExcludeFields()));
    }

    private MyRequestDto toDto(Map<String, Object> row, String language) {
