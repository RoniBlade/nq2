...
bf_settings = Variable.get("price_monitoring", deserialize_json=True)['config']
...
@dag(
    ...
    tags=['price_monitoring', 'business_files', 'target']
)
def price_monitoring_to_target():
    """
    DAG для обработки данных, запускается вручную или триггером из другого DAG.
    """

    @task
    def t_1_price_monitoring_to_target(settings_list: list):
    ...
    bf_to_target = t_1_price_monitoring_to_target.expand(settings_list=bf_settings)

    bf_to_target

price_monitoring_to_target()
