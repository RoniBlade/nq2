...
commands = [
    "echo 'Running: si_fcst_evolution_fcst' && java -jar /opt/airflow/plugins/excelLoader/excelLoader-businessfiles/si_fcst_excelLoader-release.jar /opt/airflow/plugins/excelLoader/excelLoader-businessfiles/si_fcst_evolution_fcst/si_fcst_evolution_fcst.properties /opt/airflow/plugins/excelLoader/excelLoader-businessfiles/si_fcst_evolution_fcst/log_si_fcst_evolution_fcst.properties",
]

def check_file_exists(filepath: str, **kwargs):
    if os.path.exists(filepath):
        print("Файл найден")
        return "execute_si_fcst_evolution_fcst"
    else:
...
with DAG(
    'si_fcst_evolution_fcst_to_loading',
    ...
    check_file = BranchPythonOperator(
        ...
        op_kwargs={'filepath': '/opt/airflow/s3businessfiles/SI FCST Evolution.xlsx'},
    )
    ...
    trigger_target_dag = TriggerDagRunOperator(
        ...
        trigger_dag_id='si_fcst_evolution_fcst_to_target',
        wait_for_completion=True,
    )

    final_task = DummyOperator(task_id='si_fcst_evolution_fcst_final_task')
