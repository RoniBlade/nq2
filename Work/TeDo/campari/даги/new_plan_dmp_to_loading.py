...
commands = [
    "echo 'Running: new_plan_dmp' && java -jar /opt/airflow/plugins/excelLoader/excelLoader-businessfiles/new_plan_dmp_excelLoader-release.jar /opt/airflow/plugins/excelLoader/excelLoader-businessfiles/new_plan_dmp/new_plan_dmp.properties /opt/airflow/plugins/excelLoader/excelLoader-businessfiles/new_plan_dmp/log_new_plan_dmp.properties",
]
...
with DAG(
    ...
    check_file = BranchPythonOperator(
        ...
        op_kwargs={'filepath': '/opt/airflow/s3businessfiles/NEW План ДМП неконтракт.xlsx'},
    )
    ...
    trigger_target_dag = TriggerDagRunOperator(
        ...
        trigger_dag_id='new_plan_dmp_to_target',
        wait_for_completion=True,
    )

    # Финальная задача
    final_task = DummyOperator(task_id='new_plan_dmp_final_task')
