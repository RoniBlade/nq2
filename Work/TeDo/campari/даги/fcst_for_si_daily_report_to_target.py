...
bf_settings = Variable.get("fcst_for_si_daily", deserialize_json=True)['config']
...
@dag(
    ...
    tags=['fcst_for_si_daily', 'business_files', 'target']
)
def fcst_for_si_daily_report_to_target():
    """
    DAG для обработки данных, запускается вручную или триггером из другого DAG.
    """

    @task
    def t_1_fcst_for_si_daily_to_target(settings_list: list):
    ...
    bf_to_target = t_1_fcst_for_si_daily_to_target.expand(settings_list=bf_settings)

    bf_to_target

fcst_for_si_daily_report_to_target()
