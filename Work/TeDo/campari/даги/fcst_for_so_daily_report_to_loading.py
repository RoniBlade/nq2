...
commands = [
    "echo 'Running: fcst_for_so_daily_report_fcst' && java -jar /opt/airflow/plugins/excelLoader/excelLoader-businessfiles/fcst_for_so_excelLoader-release.jar /opt/airflow/plugins/excelLoader/excelLoader-businessfiles/fcst_for_so_daily_report_fcst/fcst_for_so_daily_report_fcst.properties /opt/airflow/plugins/excelLoader/excelLoader-businessfiles/fcst_for_so_daily_report_fcst/log_fcst_for_so_daily_report_fcst.properties",
]

def check_file_exists(filepath: str, **kwargs):
    if os.path.exists(filepath):
        print("Файл найден")
        return "execute_fcst_for_so_daily_report_fcst"
    else:
...
with DAG(
    ...
    check_file = BranchPythonOperator(
        ...
        op_kwargs={'filepath': '/opt/airflow/s3businessfiles/FCST for SO Daily Report.xlsx'},
    )
    ...
    trigger_target_dag = TriggerDagRunOperator(
        ...
        trigger_dag_id='fcst_for_so_daily_report_to_target',
        wait_for_completion=True,
    )

    final_task = DummyOperator(task_id='fcst_for_so_daily_report_final_task')
