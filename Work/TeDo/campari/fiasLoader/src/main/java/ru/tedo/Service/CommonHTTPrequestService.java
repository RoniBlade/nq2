...
public class CommonHTTPrequestService {
    ...
    public CommonHttpResponseEntity processHttpGetRequest(String url, Integer timeoutSeconds, HttpProxyEntity proxy, SSLContext sslContext) throws Exception {
        return processHttpGetRequest(url, timeoutSeconds, proxy,sslContext, null);
    }

    public CommonHttpResponseEntity processHttpGetRequest(String url, Integer timeoutSeconds, HttpProxyEntity proxy, SSLContext sslContext, Header[] headers) throws Exception {
        ...
        logger.debug("HTTPs result = " + respBody);
        ...
        for (int i = 0; i < headersRes.length; i++) {
            Header h = headersRes[i];
