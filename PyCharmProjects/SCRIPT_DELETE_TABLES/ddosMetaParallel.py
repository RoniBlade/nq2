...
def get_token(email, password):
    ...
    logging.info(f"Ответ от сервера для получения токена: {response.status_code}")
...
def create_user(first_name, last_name, email, password, token):
    ...
    logging.info(f"Ответ от сервера для создания пользователя {email}: {response.status_code}")
    if response.status_code == 400:
        ...
        if "уже занят" in error_message:
            ...
            logging.error(f"Ошибка при создании пользователя {email}: {error_message}")
            response.raise_for_status()
    else:
        response.raise_for_status()
    return True
...
def run_report(email, token, report_name, report_details, code):
    ...
    try:
        ...
        return {'Пользователь': email, 'Отчет': report_name, 'Код компании': code,
                ...
                'Время выполнения (сек)': 'Ошибка формата'}
    except requests.exceptions.RequestException as re:
        logging.error(f"Ошибка запроса: {re}")
        return {'Пользователь': email, 'Отчет': report_name, 'Код компании': code,
                'Параметры': [param['value'] for param in updated_details['parameters']],
                'Время выполнения (сек)': 'Ошибка запроса'}
...
def run_reports_for_user(email, token, queries, code):
    results = []
...
# Список запросов (выберите 2 отчета)
queries = [
...
def select_and_run_reports():
        ...
        for user in users:
            ...
            if email in tokens:
                for code in ["RU03", "RU04", "RU20"]:
                    future = executor.submit(run_reports_for_user, email, tokens[email], queries, code)
    ...
    if all_results:
        ...
        df['Номер пользователя'] = df['Пользователь'].str.extract(r'(\d+)').astype(int)
        df.sort_values(by=['Номер пользователя', 'Отчет'], inplace=True)
